...
function selectRecipientType(recipientType, btn) {
  const userInput = document.getElementById('user-input');
  const groupInput = document.getElementById('group-input');
  const recipientTypeInput = document.getElementById('recipientType');
  const buttons = document.querySelectorAll('.recipient-type-btn');

  buttons.forEach(b => b.classList.remove('selected', 'bg-indigo-50', 'border-indigo-500'));
  btn.classList.add('selected', 'bg-indigo-50', 'border-indigo-500');

  if (recipientType === 'user') {
    userInput.classList.remove('hidden');
    groupInput.classList.add('hidden');
    recipientTypeInput.value = 'user';
  } else {
    userInput.classList.add('hidden');
    groupInput.classList.remove('hidden');
    recipientTypeInput.value = 'group';
  }
}
...
function openPermissionsModal() {
  ...
  if (modal) {
    ...
    const assetType = modal.dataset.assetType;
    const assetID = modal.dataset.assetId;
    const url = `/admin/assets/${assetType}/${assetID}/permissions`;
    if (typeof htmx !== 'undefined') {
      htmx.ajax('GET', url, { target: '#permissionsModal', swap: 'innerHTML' });
    }
  }
}