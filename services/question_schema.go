...
func NewQuestionSchema(cache *CacheService) *QuestionSchema {
	return &QuestionSchema{cache: cache}
}

func (s *QuestionSchema) GetSchema(ctx context.Context) (string, error) {
	var schemaJSON string
	if s.cache.Get(ctx, questionSchemaCacheKey, &schemaJSON) {
	...
	if err := reflector.AddGoComments("github.com/goquizvibe", "services/question_schema.go"); err != nil {
		return "", err
	}
	...
	_ = s.cache.Set(ctx, questionSchemaCacheKey, schemaJSON, questionSchemaTTL)
